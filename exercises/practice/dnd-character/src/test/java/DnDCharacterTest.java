...
public class DnDCharacterTest {
    ...
    public void testDiceValuesBetween1And6() {
        assertThat(dndCharacter.rollDice().stream().allMatch(d -> d >= 1 && d <= 6)).isTrue();
    }
    ...
    public void testAbilityCalculationDoesNotChangeInputScores() {
        ...
        assertThat(scores).containsExactlyElementsOf(List.of(1, 2, 3, 4));
    }
    ...
    public void testRandomCharacterIsValid() {
        for (int i = 0; i < 1000; i++) {
            ...
            assertThat(character.getStrength() > 2 && character.getStrength() < 19).isTrue();
            assertThat(character.getDexterity() > 2 && character.getDexterity() < 19).isTrue();
            assertThat(character.getConstitution() > 2 && character.getConstitution() < 19).isTrue();
            assertThat(character.getIntelligence() > 2 && character.getIntelligence() < 19).isTrue();
            assertThat(character.getWisdom() > 2 && character.getWisdom() < 19).isTrue();
            assertThat(character.getCharisma() > 2 && character.getCharisma() < 19).isTrue();
            assertThat(10 + character.modifier(character.getConstitution())).isEqualTo(character.getHitpoints());
        }
    }
